...
pub fn derive_url_scheme_plugin(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[doc(hidden)]
        #[unsafe(no_mangle)]
        pub static iris_FILE_EXT_PLUGIN: DynamicallyLinkedUrlSchemePluginInstance =
            || <#name>::new();

        static DEFAULT_TOKIO_RUNTIME: std::sync::LazyLock<iris_file_ext::prelude::thirdparty::tokio::runtime::Runtime> =
            std::sync::LazyLock::new(|| {
                iris_url_scheme::prelude::thirdparty::tokio::runtime::Runtime::new().expect("Failed to create Tokio runtime")
            });

        fn default_runtime<T: Send + 'static>(
            task: impl Future<Output = T> + Send + 'static,
        ) -> iris_url_scheme::prelude::thirdparty::tokio::task::JoinHandle<T> {
            match iris_url_scheme::prelude::thirdparty::tokio::runtime::Handle::try_current() {
        ...
        if let ImplItem::Fn(method) = item {
            ...
            if was_async {
                let old_return_type = match &method.sig.output {
                    ...
                    ReturnType::Type(_, ty) => {
                        if method.sig.ident == "new" {
                            quote! { iris_url_scheme::prelude::thirdparty::eyre::Result<Box<dyn UrlSchemePlugin>> }
                        } else {
                            quote! { #ty }
                        }
                    }
                ...
                method.sig.output = syn::parse_quote! {
                    -> iris_url_scheme::prelude::thirdparty::tokio::task::JoinHandle<#old_return_type>
                };
