...
pub fn derive_node(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[doc(hidden)]
        #[unsafe(no_mangle)]
        pub static iris_NODE: iris_api::prelude::DynamicallyLinkedNodeInstance = |inputs, outputs, queries, queryables, configuration| {
            <#name>::new(inputs, outputs, queries, queryables, configuration)
        };

        static DEFAULT_TOKIO_RUNTIME: std::sync::LazyLock<iris_api::prelude::thirdparty::tokio::runtime::Runtime> =
            std::sync::LazyLock::new(|| iris_api::prelude::thirdparty::tokio::runtime::Runtime::new().expect("Failed to create Tokio runtime"));

        fn default_runtime<T: Send + 'static>(
            task: impl Future<Output = T> + Send + 'static,
        ) -> iris_api::prelude::thirdparty::tokio::task::JoinHandle<T> {
            match iris_api::prelude::thirdparty::tokio::runtime::Handle::try_current() {
                ...
                let old_return_type = match &method.sig.output {
                    ...
                    ReturnType::Type(_, ty) => {
                        if method.sig.ident == "new" {
                            quote! { iris_api::prelude::thirdparty::eyre::Result<Box<dyn Node>> }
                        } else {
                            quote! { #ty }
                        }
                    }
